...
- A local-Igra bridge rehearsal using fresh writable Pearl simnet reserve
  deposit and release txids from the 2026-05-20 multisig proof.
...
2. Select reserve addresses, signer policy, and hot/warm/cold limits.
   - The service has cap checks and alerting, but no approved live reserve
     address set or custody policy is recorded.
   - 2026-05-20 update: the pilot reserve custody policy is drafted in
     `docs/operations/bridge-reserve-custody-policy.md`.
   - Next action: select the live reserve addresses, signer identities, emergency
     pause authority, daily limits, and maximum hot-wallet exposure.

3. Decide and implement bridge reserve custody.
   ...
   - Evidence: funding tx
     ...
     spent it.
   - 2026-05-20 update: the simnet scanner now classifies the fresh reserve
     release as `exit_release`; the bridge rehearsal consumed that evidence,
     matched the reserve spend to the mirrored exit, and reconciled with no
     blockers.
   - Next action: approve live reserve addresses and signer custody before any
     live exit.
...
1. Custody policy PR: reserve addresses, signer tiers, emergency pause drill,
...
2. Deployment proof PR when Galleon/Igra testnet is back: fix the fee/gas path
   or use a replacement deploy route and commit evidence.
3. Low-cap mainnet proof PR: real Pearl mainnet and Base/Igra txids after
   explicit approval, with public proof and clean reconciliation.
4. Bridge proof UI PR: public bridge deposit/exit/reserve proof pages.
5. Prod Oyster release PR: prod secrets, prod image path, prod DNS, and smoke
   evidence.
