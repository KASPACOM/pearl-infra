...
test('fails production startup when persistence, RPC, real Pearl escrow, or contract config is missing', () => {
  ...
  assert.throws(
    ...
    /OTC_API_DATABASE_URL.*BASE_RPC_URL.*PEARL_ESCROW_ALLOCATOR=p2tr_xpub or p2tr_multisig.*PEARL_INDEXER_WATCH_URL.*OTC_ADMIN_API_TOKEN or OTC_ADMIN_API_TOKENS.*OTC_ALERT_WEBHOOK_URL.*BASE_USDC_ESCROW_CONTRACT/,
  );
});

test('accepts multi-token RBAC admin config for production startup', () => {
  const config = readOtcApiConfig({
    ...
    OTC_ADMIN_API_TOKENS: 'admin-token:admin-user:admin;support-token:support-user:support_read',
...
test('accepts multisig Pearl escrow production startup config', () => {
  const config = readOtcApiConfig({
    OTC_API_REQUIRE_PRODUCTION_CONFIG: 'true',
    OTC_API_DATABASE_URL: 'postgres://user:pass@db/otc',
    BASE_RPC_URL: 'https://base-sepolia.example',
    PEARL_ESCROW_ALLOCATOR: 'p2tr_multisig',
    PEARL_ESCROW_ARBITER_PUBKEY: '11'.repeat(32),
    PEARL_INDEXER_WATCH_URL: 'http://pearl-indexer:8080',
    BASE_USDC_ESCROW_CONTRACT: '0x1111111111111111111111111111111111111111',
    OTC_ADMIN_API_TOKEN: 'test-admin-token',
    OTC_ALERT_WEBHOOK_URL: 'https://alerts.example.test/otc',
...
test('requires an arbiter pubkey for multisig Pearl escrow production startup', () => {
  const config = readOtcApiConfig({
    OTC_API_REQUIRE_PRODUCTION_CONFIG: 'true',
    OTC_API_DATABASE_URL: 'postgres://user:pass@db/otc',
    BASE_RPC_URL: 'https://base-sepolia.example',
...
test('requires email webhook when notification worker is enabled in production', () => {
  const baseEnv = {
    OTC_API_REQUIRE_PRODUCTION_CONFIG: 'true',
    OTC_API_DATABASE_URL: 'postgres://user:pass@db/otc',
    BASE_RPC_URL: 'https://base-sepolia.example',
    ...
    OTC_ADMIN_API_TOKEN: 'test-admin-token',
    OTC_ALERT_WEBHOOK_URL: 'https://alerts.example.test/otc',
    OTC_NOTIFICATION_WORKER_ENABLED: 'true',
  };
  const runtime = readOtcApiRuntimeConfig({ OTC_API_REQUIRE_PRODUCTION_CONFIG: 'true' });

  assert.throws(
    () => assertOtcApiStartupConfig(readOtcApiConfig(baseEnv), runtime),
    /OTC_NOTIFICATION_EMAIL_WEBHOOK_URL/,
  );
  assert.doesNotThrow(() =>
    assertOtcApiStartupConfig(
      readOtcApiConfig({ ...baseEnv, OTC_NOTIFICATION_EMAIL_WEBHOOK_URL: 'https://email.example.test/send' }),
      runtime,
    ),
  );
});
