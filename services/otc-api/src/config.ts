...
export function readOtcApiConfig(env: NodeJS.ProcessEnv = process.env): OtcApiConfig {
  return {
    ...
    pearlPrefundEnabled: env.PEARL_PREFUND_ENABLED === 'true',
    pearlPrefundOperatorPubkey: env.PEARL_PREFUND_OPERATOR_PUBKEY,
    pearlPrefundDerivationPrefix: env.PEARL_PREFUND_DERIVATION_PREFIX ?? '1',
    pearlPrefundRefundDelayHours: env.PEARL_PREFUND_REFUND_DELAY_HOURS ? Number(env.PEARL_PREFUND_REFUND_DELAY_HOURS) : 24,
